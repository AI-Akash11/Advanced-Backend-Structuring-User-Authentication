...
var config = {
  ...
  access_duration: process.env.ACCESS_DURATION,
  refresh_duration: process.env.REFRESH_DURATION
};
  ...
  return async (req, res, next) => {
    try {
      ...
      if (roles.length && !roles.includes(user.role)) {
        res.status(403).json({
          success: false,
          message: "Forbidden Access for role!"
        });
      }
      req.user = decoded;
      next();
...
var loginUserIntoDB = async (payload) => {
  ...
  const accessToken = jwt2.sign(jwtpayload, config_default.secret, { expiresIn: config_default.access_duration });
  const refreshToken2 = jwt2.sign(jwtpayload, config_default.refresh_secret, { expiresIn: config_default.refresh_duration });
